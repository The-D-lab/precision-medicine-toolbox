...
class DataSet:
    ...
    def __init__(self, data_path: str = None,
                 ...
                 image_names: list = ['image', 'volume', 'img', 'vol'], twod_image=False):
            ...
            for patient in tqdm(self.__patients):
                if self._twod_image:
                    ...
                    if not len(dcm_files):
                        warn(
                            'No dcm data found for patient:%s check the folder, ensure that dicom files ends with .dcm' % patient)
                                    ...
                                    if temp_file.Modality == 'RTSTRUCT':
                                        ...
                                        for dfile in dcm_files:
                                            ...
                                            if is_in_list(['ct', 'mr', 'us', 'nm'], str(temp_dfile.Modality).lower()):
                                                ...
                                                self._patient_dict[patient + '_' + rts_name[-15:]] = [datafile,
                                                                                                      structfile]
                                                break
